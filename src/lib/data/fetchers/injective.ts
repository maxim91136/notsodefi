/**
 * Injective (INJ) Data Fetcher
 *
 * Cosmos SDK chain focused on DeFi. Uses Tendermint consensus.
 * No API key required.
 *
 * API: https://sentry.lcd.injective.network
 */
...
export class InjectiveFetcher {
  private baseUrl: string;

  constructor(baseUrl?: string) {
    this.baseUrl = baseUrl || INJECTIVE_LCD_URL;
  }
  ...
  private async fetchEndpoint<T>(endpoint: string): Promise<T> {
    ...
    if (!response.ok) {
      throw new Error(`Injective API error: ${response.status}`);
    }
  ...
  async getValidatorStats(): Promise<{
    ...
    // Convert from base units to INJ (18 decimals)
  ...
  async getAllMetrics(): Promise<InjectiveMetrics> {
    try {
      ...
      console.error('Failed to fetch Injective metrics:', error);
      return {
...
export function getInjectiveFetcher(baseUrl?: string): InjectiveFetcher {
  return new InjectiveFetcher(baseUrl);
}
