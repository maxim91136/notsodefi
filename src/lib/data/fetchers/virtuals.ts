 ...
 * Key metrics: Market cap, fees/revenue across Base/Ethereum/Solana.
 *
...
export class VirtualsFetcher {
   ...
   */
  async getProtocolData(): Promise<{ marketCap: number | null }> {
    try {
      const response = await fetch(`${DEFILLAMA_API}/protocol/virtuals-protocol`);
      if (!response.ok) {
  ...
  async getAllMetrics(): Promise<VirtualsMetrics> {
    const [protocolData, feesData] = await Promise.all([
      this.getProtocolData(),
      this.getFees(),
    ]);

    return {
      marketCap: protocolData.marketCap,
